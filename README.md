...
Install Aurelius as a development dependency in the project that owns the documentation. This records the version in `package.json` and locks it in `package-lock.json`.

When `@avw/aurelius` is published to npm:

```bash
npm install --save-dev @avw/aurelius@^0.4.0
```

Until it is published, install a tagged Git release:
